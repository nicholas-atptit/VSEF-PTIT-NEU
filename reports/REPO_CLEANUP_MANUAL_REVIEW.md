...
1. `reports/VN100_HYBRID_BENCHMARK_CLOSEOUT.md` - final-looking VN100 evidence, but not part of the current active five-track evidence index.
2. `reports/NCKH_EVIDENCE_GAP_CLOSURE_REGISTER.md` - may contain evidence traceability, so it was kept.
3. `reports/NCKH_EVIDENCE_UPGRADE_ADDENDUM_V2.md` - may contain current claim-boundary notes, so it was kept.
4. `reports/NCKH_EXPERIMENT_INVENTORY.md` - references old paper artifact builders but may still serve as historical experiment index.
5. `reports/NCKH_RESULTS_CLAIM_REGISTER.md` - superseded status unclear because a V2 also exists.
6. `reports/NCKH_RESULTS_CLAIM_REGISTER_V2.md` - may be evidence-adjacent and should be reviewed against the active claim registers.
7. `reports/NCKH_RESEARCH_DESIGN_VN100.md` - old VN100 design scope but could still explain historical experiments.
8. `reports/VN30_HOURLY_2015_JAN2025_BENCHMARK_RESULT_SUMMARY.md` - universe-specific evidence; kept until reviewed against current data-forensics scope.
9. `reports/VN30_HOURLY_2015_JAN2025_UNIVERSE_DECISION.md` - may still explain universe membership.
10. `reports/VN30_HOURLY_2015_POST_BENCHMARK_DIAGNOSTICS_SUMMARY.md` - diagnostic evidence; kept rather than archived.
