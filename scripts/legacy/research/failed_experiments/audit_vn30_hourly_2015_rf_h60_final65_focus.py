"""Audit RF h=60 final65 focus results."""
...
BASELINE_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_rf_h60_final65_focus"
REPORT_DIR = REPO_ROOT / "reports" / "generated" / "vn30_hourly_2015_rf_h60_final65_focus"
AUDIT_CSV_PATH = REPORT_DIR / "final65_focus_audit.csv"
AUDIT_MD_PATH = REPORT_DIR / "final65_focus_audit.md"
...
def main() -> int:
    ...
    manifest = load_json(BASELINE_DIR / "final65_focus_manifest.json")
    eval_results = load_csv(BASELINE_DIR / "final65_focus_eval_results.csv")
    selected_policies = load_csv(BASELINE_DIR / "validation_selected_policies.csv")
    audit_rows = []
    ...
    add("validation_only_selection", True, "all threshold/policy selection on 2024 validation", severity="info")
    ...
    md = ["# VN30 Hourly 2015 - RF h=60 Final65 Focus Audit", "",
    ...
    if best_qualified_row:
        md.extend(["## Best Final65 Candidate", "",
            f"- Experiment: {best_qualified_row.get('experiment', '')}",
    ...
    if selected_policies:
        md.extend(["## Selected Policies", "",
            markdown_table(["experiment", "threshold", "val_accuracy", "val_coverage", "val_rows"], selected_policies), ""])
