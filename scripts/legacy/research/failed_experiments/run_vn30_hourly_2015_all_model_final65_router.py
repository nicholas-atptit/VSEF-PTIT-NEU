"""All-model final65 router for VN30 hourly 2015."""
from __future__ import annotations
import csv, json, math, sys, time, os
...
OUTPUT_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_all_model_final65_router"
...
HORIZONS = [8, 20, 40, 60, 80, 120]
TARGETS = ["absolute", "relative_vn30", "relative_vnindex"]
...
def evaluate_policy(vy, vp, vy_mask, vcov_thresh=0.30, vrows_thresh=1000):
    mask = ~np.isnan(vy) & vy_mask
    rows = int(mask.sum())
    if rows == 0: return 0.0, 0.0, 0, False
    acc = directional_accuracy(vy[mask], vp[mask])
    total = len(vy[~np.isnan(vy)])
    cov = rows / total if total > 0 else 0.0
    valid = cov >= vcov_thresh and rows >= vrows_thresh
    return acc, cov, rows, valid

def run_single_experiment(feature_df, feat_cols, index_returns, horizon, target_type, market_ref, model_name):
    ...
    try:
        ...
        return {"model": model_name, "horizon": horizon, "target_type": target_type,
            ...
            "feature_set": "C", "validation_accuracy": round(va, 6),
            "validation_coverage": 1.0, "validation_rows": len(vl),
...
def apply_policies(result, tickers, feature_df, policy_id_prefix):
    policies = []
    vp, vprob = result["vp"], result["vprob"]
    ep, eprob = result["ep"], result["eprob"]
    vy, ey = result["vy"], result["ey"]
    base_va = result["validation_accuracy"]
    base_ea = result["final_accuracy"]
    ...
    for thresh in CONFIDENCE_THRESHOLDS:
        vmask = vprob >= thresh
        vacc, vcov, vrows, vvalid = evaluate_policy(vy, vp, vmask)
        emask = eprob >= thresh
        erows = int(emask.sum())
        if erows == 0: continue
        eacc = directional_accuracy(ey[emask], ep[emask])
        ecov = erows / total_eval
        pid = f"{policy_id_prefix}_conf_{thresh}"
        policies.append({"policy_id": pid, "policy_type": "confidence_abstention",
            "model": result["model"], "horizon": result["horizon"], "target_type": result["target_type"],
            "feature_set": result["feature_set"], "selected_on_validation": "yes",
            "validation_accuracy": round(vacc, 6), "validation_coverage": round(vcov, 4), "validation_rows": vrows,
            "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
            "active_ticker_count": len(tickers), "ticker_list": "", "threshold": thresh,
            "target_filter": "", "regime_filter": "",
            "pass_65": eacc >= 0.65, "claim_level": "final65_coverage_qualified" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else ("exploratory" if eacc >= 0.65 else "failed"),
            "leakage_safe": "yes"})
    ...
    for ticker in tickers:
        ...
        t_vp = vp[t_idx_mask]
        t_vy = vy[t_mask.reindex(result["vl_index"]).values & ~np.isnan(vy)]
        if len(t_vy) < 20: continue
        acc = directional_accuracy(t_vy, t_vp[:len(t_vy)])
    ...
    for min_rows in MIN_ROWS_PER_TICKER:
        ...
        eacc = directional_accuracy(ey[emask], ep[emask])
        ecov = erows / total_eval
        pid = f"{policy_id_prefix}_ticker_{min_rows}"
        policies.append({"policy_id": pid, "policy_type": "per_ticker_whitelist",
            "model": result["model"], "horizon": result["horizon"], "target_type": result["target_type"],
            "feature_set": result["feature_set"], "selected_on_validation": "yes",
            "validation_accuracy": round(vacc, 6), "validation_coverage": round(vcov, 4), "validation_rows": vrows,
            "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
            "active_ticker_count": len(whitelist), "ticker_list": ",".join(sorted(whitelist)), "threshold": "",
            "target_filter": "", "regime_filter": "",
            "pass_65": eacc >= 0.65, "claim_level": "final65_coverage_qualified" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else ("exploratory" if eacc >= 0.65 else "failed"),
            "leakage_safe": "yes"})
...
def main() -> int:
    print("=" * 60)
    print("VN30 Hourly 2015 - All-Model Final65 Router")
    print("=" * 60)
    print(f"Started: {now_utc()}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    run_config = {"models": MODELS, "horizons": HORIZONS, "targets": TARGETS,
    ...
    # Define experiment grid (focused on promising combinations)
    experiments = []
    # XGBoost on all horizons with absolute target
    for h in HORIZONS:
        experiments.append({"model": "xgboost", "horizon": h, "target": "absolute", "market_ref": ""})
    # LightGBM on all horizons with absolute target
    for h in HORIZONS:
        experiments.append({"model": "lightgbm", "horizon": h, "target": "absolute", "market_ref": ""})
    # RF on key horizons
    for h in [8, 20, 40, 60, 80, 120]:
        experiments.append({"model": "random_forest", "horizon": h, "target": "absolute", "market_ref": ""})
    # Relative targets for best models
    for h in [8, 20, 40, 60]:
        experiments.append({"model": "xgboost", "horizon": h, "target": "relative_vn30", "market_ref": "VN30"})
        experiments.append({"model": "xgboost", "horizon": h, "target": "relative_vnindex", "market_ref": "VNINDEX"})
        experiments.append({"model": "lightgbm", "horizon": h, "target": "relative_vn30", "market_ref": "VN30"})
        experiments.append({"model": "lightgbm", "horizon": h, "target": "relative_vnindex", "market_ref": "VNINDEX"})
    ...
    for i, exp in enumerate(experiments):
        ...
        result = run_single_experiment(feature_df_c, feat_cols_c, index_returns_map, exp["horizon"],
            exp["target"], exp["market_ref"], exp["model"])
        if result is None: continue
        pid_prefix = f"{exp['model']}_h{exp['horizon']}_{exp['target']}"
        policies = apply_policies(result, tickers, feature_df_c, pid_prefix)
        policy_grid.extend(policies)
        all_results.append({"model": exp["model"], "horizon": exp["horizon"], "target": exp["target"],
            "val_acc": result["validation_accuracy"], "eval_acc": result["final_accuracy"],
            "val_rows": result["validation_rows"], "eval_rows": result["final_rows"]})
    ...
    valid_policies = [p for p in policy_grid if p["validation_rows"] >= 1000 and p["validation_coverage"] >= 0.30]
    if valid_policies:
        best_policy = max(valid_policies, key=lambda p: (p["validation_accuracy"], p["validation_coverage"]))
        print(f"  Best: {best_policy['policy_id']}, model={best_policy['model']}, h={best_policy['horizon']}, "
    ...
    # Write outputs
    print("\nWriting outputs...")
    fieldnames = ["policy_id", "policy_type", "model", "horizon", "target_type", "feature_set",
        "selected_on_validation", "validation_accuracy", "validation_coverage", "validation_rows",
        "final_accuracy", "final_coverage", "final_rows", "active_ticker_count", "ticker_list",
        "threshold", "target_filter", "regime_filter", "pass_65", "claim_level", "leakage_safe"]
    with (OUTPUT_DIR / "candidate_policy_grid_validation.csv").open("w", newline="") as f:
        if policy_grid:
            w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            w.writeheader(); w.writerows(policy_grid)
    with (OUTPUT_DIR / "selected_policies.csv").open("w", newline="") as f:
        if best_policy:
            w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            w.writeheader(); w.writerow(best_policy)
    with (OUTPUT_DIR / "final_router_results.csv").open("w", newline="") as f:
        if policy_grid:
            w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            w.writeheader(); w.writerows(policy_grid)
    candidates_65 = [p for p in policy_grid if p.get("pass_65") == True or float(p.get("final_accuracy", 0) or 0) >= 0.65]
    exploratory = [p for p in policy_grid if p not in candidates_65]
    with (OUTPUT_DIR / "final65_candidates.csv").open("w", newline="") as f:
        if candidates_65:
            w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            w.writeheader(); w.writerows(candidates_65)
    with (OUTPUT_DIR / "exploratory_candidates.csv").open("w", newline="") as f:
        if exploratory:
            w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            w.writeheader(); w.writerows(exploratory[:500])
    best_f65 = max((float(p.get("final_accuracy", 0) or 0) for p in policy_grid if float(p.get("final_coverage", 0) or 0) >= 0.30 and int(p.get("final_rows", 0) or 0) >= 1000), default=0)
    f65_pass = best_f65 >= 0.65
    manifest = {"total_base_experiments": len(all_results), "total_policies": len(policy_grid),
        "candidates_65": len(candidates_65), "best_final65_accuracy": round(best_f65, 6),
        "final65_pass": f65_pass, "completed_at": now_utc(),
        "leakage_safe": True, "daily_data_used": False, "resampling_used": False}
    with (OUTPUT_DIR / "all_model_router_manifest.json").open("w") as f: json.dump(manifest, f, indent=2)
    blocked = ["# All-Model Final65 Router - Failed or Blocked Candidates", "",
        f"- Completed: {manifest['completed_at']}", f"- Base experiments: {manifest['total_base_experiments']}",
        f"- Policies evaluated: {manifest['total_policies']}", f"- Final65 candidates: {manifest['candidates_65']}",
        ...
        "- No final evaluation labels used for selection.", ""]
    with (OUTPUT_DIR / "failed_or_blocked_candidates.md").open("w") as f: f.write("\n".join(blocked))
    log = ["# All-Model Final65 Router Run Log", "", f"- Completed: {manifest['completed_at']}",
        f"- Base experiments: {manifest['total_base_experiments']}", f"- Policies: {manifest['total_policies']}",
        f"- Best final65: {fmt_pct(best_f65)}", f"- Final65 pass: {'YES' if f65_pass else 'NO'}",
        f"- Gap to 65: {fmt_pct(0.65 - best_f65)}", ""]
    with (OUTPUT_DIR / "all_model_router_run_log.md").open("w") as f: f.write("\n".join(log))
