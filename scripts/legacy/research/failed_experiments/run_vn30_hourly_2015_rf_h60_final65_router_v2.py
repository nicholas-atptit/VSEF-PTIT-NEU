"""RF h=60 Final65 Router v2 for VN30 hourly 2015."""
...
OUTPUT_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_rf_h60_final65_router_v2"
...
def evaluate_policy(vy, vp, vy_mask, vcov_thresh=0.30, vrows_thresh=1000):
    """Evaluate a policy on validation, return (acc, cov, rows, valid)"""
...
def main() -> int:
    print("=" * 60)
    print("VN30 Hourly 2015 - RF h=60 Final65 Router v2")
    print("=" * 60)
    ...
    for thresh in CONFIDENCE_THRESHOLDS:
        vmask = vprob >= thresh
        vacc, vcov, vrows, vvalid = evaluate_policy(vy.values, vp, vmask)
        emask = eprob >= thresh
        erows = int(emask.sum())
        if erows == 0: continue
        eacc = directional_accuracy(ey.values[emask], ep[emask])
        ecov = erows / len(ey)
        all_policies.append({"policy_type": "confidence_abstention", "threshold": thresh,
            "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
            "validation_coverage": round(vcov, 4), "validation_rows": vrows})
        all_results.append({"policy_type": "confidence_abstention", "threshold": thresh,
            "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
            "validation_coverage": round(vcov, 4), "validation_rows": vrows,
            "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
            "active_tickers": len(tickers), "ticker_list": "", "regime_filter": "",
            "pass_65": eacc >= 0.65, "claim_level": "conditional_final65_candidate" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else "exploratory_only"})
    ...
    for ticker in tickers:
        ...
        t_vl = vl[t_mask.reindex(vl.index)]
        if len(t_vl) < 50: continue
        t_vX = feature_df_c.reindex(t_vl.index)[fcp].fillna(0)
        t_vy = t_vl.astype(int)
        # Use base model predictions for this ticker
        t_idx_mask = t_mask.reindex(vl.index).values
        t_vp = vp[t_idx_mask]
        t_vprob = vprob[t_idx_mask]
        acc = directional_accuracy(t_vy.values, t_vp)
        ticker_val_acc[ticker] = (acc, len(t_vl))
    for min_rows in MIN_ROWS_PER_TICKER:
        whitelist = [t for t, (a, n) in ticker_val_acc.items() if n >= min_rows]
        if not whitelist: continue
        vmask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in vl.index]) & ~np.isnan(vy.values)
        vacc, vcov, vrows, vvalid = evaluate_policy(vy.values, vp, vmask)
        emask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in el.index]) & ~np.isnan(ey.values)
        erows = int(emask.sum())
        if erows == 0: continue
        eacc = directional_accuracy(ey.values[emask], ep[emask])
        ecov = erows / len(ey)
        all_policies.append({"policy_type": "per_ticker_whitelist", "threshold": "",
            "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
            "validation_coverage": round(vcov, 4), "validation_rows": vrows})
        all_results.append({"policy_type": "per_ticker_whitelist", "threshold": "",
            "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
            "validation_coverage": round(vcov, 4), "validation_rows": vrows,
            "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
            "active_tickers": len(whitelist), "ticker_list": ",".join(sorted(whitelist)), "regime_filter": "",
            "pass_65": eacc >= 0.65, "claim_level": "conditional_final65_candidate" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else "exploratory_only"})

    # Policy C: Ticker + Confidence
    print("\n[Policy C] Ticker + confidence policy...")
    for min_rows in MIN_ROWS_PER_TICKER:
        whitelist = [t for t, (a, n) in ticker_val_acc.items() if n >= min_rows]
        if not whitelist: continue
        for thresh in CONFIDENCE_THRESHOLDS:
            vmask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in vl.index]) & (vprob >= thresh) & ~np.isnan(vy.values)
            vacc, vcov, vrows, vvalid = evaluate_policy(vy.values, vp, vmask)
            emask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in el.index]) & (eprob >= thresh) & ~np.isnan(ey.values)
            erows = int(emask.sum())
            if erows == 0: continue
            eacc = directional_accuracy(ey.values[emask], ep[emask])
            ecov = erows / len(ey)
            all_policies.append({"policy_type": "ticker_confidence", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows})
            all_results.append({"policy_type": "ticker_confidence", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows,
                "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
                "active_tickers": len(whitelist), "ticker_list": ",".join(sorted(whitelist)), "regime_filter": "",
    ...
    if regime_features:
        ...
        for thresh in [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]:
            ...
            eacc = directional_accuracy(ey.values[emask], ep[emask])
            ecov = erows / len(ey)
            all_policies.append({"policy_type": "market_regime_abstention", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows})
            all_results.append({"policy_type": "market_regime_abstention", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows,
                "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
                "active_tickers": len(tickers), "ticker_list": "", "regime_filter": ",".join(regime_features),
                "pass_65": eacc >= 0.65, "claim_level": "conditional_final65_candidate" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else "exploratory_only"})

    # Policy E: Ticker + Regime + Confidence Router
    print("\n[Policy E] Ticker + Regime + Confidence router...")
    for min_rows in MIN_ROWS_PER_TICKER:
        whitelist = [t for t, (a, n) in ticker_val_acc.items() if n >= min_rows and a >= 0.55]
        if not whitelist: continue
        for thresh in [0.5, 0.55, 0.6, 0.65, 0.7]:
            vmask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in vl.index]) & (rmvpp >= thresh) & ~np.isnan(vy.values)
            vacc, vcov, vrows, vvalid = evaluate_policy(vy.values, vp, vmask)
            emask = np.array([feature_df_c.loc[i, "ticker"] in whitelist for i in el.index]) & (rmpp >= thresh) & ~np.isnan(ey.values)
            erows = int(emask.sum())
            if erows == 0: continue
            eacc = directional_accuracy(ey.values[emask], ep[emask])
            ecov = erows / len(ey)
            all_policies.append({"policy_type": "ticker_regime_confidence", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows})
            all_results.append({"policy_type": "ticker_regime_confidence", "threshold": thresh,
                "selected_on_validation": "yes", "validation_accuracy": round(vacc, 6),
                "validation_coverage": round(vcov, 4), "validation_rows": vrows,
                "final_accuracy": round(eacc, 6), "final_coverage": round(ecov, 4), "final_rows": erows,
                "active_tickers": len(whitelist), "ticker_list": ",".join(sorted(whitelist)), "regime_filter": ",".join(regime_features),
                "pass_65": eacc >= 0.65, "claim_level": "conditional_final65_candidate" if eacc >= 0.65 and ecov >= 0.30 and erows >= 1000 else "exploratory_only"})

    # Select best policy on validation
    print("\nSelecting best policy on validation...")
    valid_policies = [r for r in all_results if r["validation_rows"] >= 1000 and r["validation_coverage"] >= 0.30]
    if valid_policies:
        best_policy = max(valid_policies, key=lambda r: (r["validation_accuracy"], r["validation_coverage"]))
        print(f"  Best: {best_policy['policy_type']}, val_acc={fmt_pct(best_policy['validation_accuracy'])}, val_cov={fmt_pct(best_policy['validation_coverage'])}")
    else:
        best_policy = None
    ...
    candidates_65 = [r for r in all_results if r.get("pass_65") == True or float(r.get("final_accuracy", 0) or 0) >= 0.65]
    exploratory = [r for r in all_results if r not in candidates_65]
    with (OUTPUT_DIR / "router_v2_candidates_65.csv").open("w", newline="") as f:
        if candidates_65:
            w = csv.DictWriter(f, fieldnames=all_results[0].keys(), extrasaction="ignore")
            w.writeheader(); w.writerows(candidates_65)
    with (OUTPUT_DIR / "router_v2_exploratory_candidates.csv").open("w", newline="") as f:
        if exploratory:
            w = csv.DictWriter(f, fieldnames=all_results[0].keys(), extrasaction="ignore")
            w.writeheader(); w.writerows(exploratory[:200])
    best_f65 = max((float(r.get("final_accuracy", 0) or 0) for r in all_results if float(r.get("final_coverage", 0) or 0) >= 0.30 and int(r.get("final_rows", 0) or 0) >= 1000), default=0)
    f65_pass = best_f65 >= 0.65
    manifest = {"total_policies": len(all_policies), "total_results": len(all_results),
    ...
    with (OUTPUT_DIR / "router_v2_manifest.json").open("w") as f: json.dump(manifest, f, indent=2)
    log = ["# RF h=60 Final65 Router v2 Run Log", "", f"- Completed: {manifest['completed_at']}",
        f"- Policies: {manifest['total_policies']}", f"- Results: {manifest['total_results']}",
        f"- Best final65: {fmt_pct(best_f65)}", f"- Final65 pass: {'YES' if f65_pass else 'NO'}",
        f"- Gap to 65: {fmt_pct(0.65 - best_f65)}", ""]
    with (OUTPUT_DIR / "router_v2_run_log.md").open("w") as f: f.write("\n".join(log))
