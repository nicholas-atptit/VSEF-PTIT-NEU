...
BASELINE_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_target_redesign_experiments"
REPORT_DIR = REPO_ROOT / "reports" / "generated" / "vn30_hourly_2015_target_redesign"
AUDIT_CSV_PATH = REPORT_DIR / "target_redesign_audit.csv"
AUDIT_MD_PATH = REPORT_DIR / "target_redesign_audit.md"
...
def main() -> int:
    ...
    manifest = load_json(BASELINE_DIR / "target_redesign_manifest.json")
    eval_results = load_csv(BASELINE_DIR / "final_target_results.csv")
    coverage_summary = load_csv(BASELINE_DIR / "target_coverage_summary.csv")
    selected_policy = load_csv(BASELINE_DIR / "selected_target_policy.csv")
    audit_rows = []
    ...
    global_results = [r for r in eval_results if r.get("full_universe_result") == "yes"]
    qualified_results = [r for r in eval_results if r.get("coverage_qualified") == "yes"]
    ...
    md = ["# VN30 Hourly 2015 - Target Redesign Audit", "",
    ...
    if best_global_row:
        md.extend(["## Best Global Candidate", "",
            f"- Target: {best_global_row.get('target_type', '')}, Threshold: {best_global_row.get('target_threshold', '')}",
    ...
    if best_qualified_row:
        md.extend(["## Best Coverage-Qualified Candidate", "",
            f"- Target: {best_qualified_row.get('target_type', '')}, Threshold: {best_qualified_row.get('target_threshold', '')}",
