"""Build benchmark-readiness manifest from gateway validation outputs."""
...
INDEX_VALIDATION = REPO_ROOT / "reports" / "generated" / "index_hourly_gateway" / "validation" / "index_hourly_gateway_validation.csv"
STOCK_VALIDATION = REPO_ROOT / "reports" / "generated" / "vn30_hourly_gateway" / "validation" / "vn30_hourly_gateway_validation.csv"
CLEAN_MANIFEST = REPO_ROOT / "reports" / "generated" / "vn30_hourly_clean_workspace" / "clean_workspace_manifest.json"
REPORT_ROOT = REPO_ROOT / "reports" / "generated" / "vn30_gateway_benchmark_readiness"
JSON_PATH = REPORT_ROOT / "vn30_gateway_benchmark_readiness_manifest.json"
MD_PATH = REPORT_ROOT / "vn30_gateway_benchmark_readiness_report.md"
...
def main() -> int:
    ...
    train_start_by_ticker = {row.get("ticker", ""): row.get("first_datetime", "") for row in stocks}
    ...
    payload: dict[str, Any] = {
        ...
        "actual_train_start_per_ticker": train_start_by_ticker,
        ...
        "benchmark_command_later": (
            "<repo-approved-venv-python> scripts\\research\\run_vn30_hourly_benchmark_2005_2026_from_fetched.py"
            if benchmark_ready
            else ""
        ),
        ...
        "clean_workspace_manifest": clean_payload,
    }
    REPORT_ROOT.mkdir(parents=True, exist_ok=True)
    JSON_PATH.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# VN30 Gateway Benchmark Readiness",
        "",
        ...
        f"- Actual eval start: `{payload['actual_eval_start']}`.",
        ...
        "",
        "## Per-Ticker Actual Starts",
        "",
        "| ticker | first datetime |",
        "|---|---|",
    ]
    for ticker, first in sorted(train_start_by_ticker.items()):
        lines.append(f"| `{ticker}` | {first} |")
