"""Run confidence-threshold diagnostics for vnstock-fetched VN30 hourly benchmark."""
...
DEFAULT_OUTPUT_DIR = FULL_REPORT_ROOT / "confidence"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VN30 hourly vnstock confidence sweep.")
...
def write_report(path: Path, artifact_dir: Path, sweep: pd.DataFrame) -> None:
    ...
    content = [
        "# VN30 Hourly vnstock Confidence Sweep Report",
        "",
        "## Source",
        "",
    ...
    if sweep.empty:
        content.append("No confidence-sweep claims are available because vnstock full-benchmark predictions are missing or empty.")
    else:
...
def write_plot(path: Path, sweep: pd.DataFrame) -> str:
    if sweep.empty:
        return save_placeholder_figure(path, "VN30 hourly vnstock confidence sweep", "No hourly predictions were available.")
    try:
        import matplotlib
        ...
        axes[0].set_title("VN30 hourly vnstock confidence sweep")
        ...
        return save_placeholder_figure(path, "VN30 hourly vnstock confidence sweep", f"Plot rendering failed: {exc}")
...
def main() -> int:
    ...
    summary_path = args.output_dir / "vn30_hourly_vnstock_confidence_sweep_summary.csv"
    report_path = args.output_dir / "vn30_hourly_vnstock_confidence_sweep_report.md"
    figure_path = args.output_dir / "vn30_hourly_vnstock_confidence_threshold_coverage_accuracy.png"
    ...
    print(
        "VN30 hourly vnstock confidence sweep complete: "
