"""Audit the VN30 Daily 2015 Target60 optimization outputs."""
...
REPORT_ROOT = REPO_ROOT / "reports" / "generated" / "vn30_daily_2015_target60"
AUDIT_CSV_PATH = REPORT_ROOT / "daily_target60_audit.csv"
AUDIT_MD_PATH = REPORT_ROOT / "daily_target60_audit.md"
...
def generated_paths(output_dir: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "daily_manifest": output_dir / "daily_target60_manifest.json",
        "validation_results": daily / "validation_candidate_results.csv",
        "final_results": daily / "final_candidate_results.csv",
        "best_candidates": daily / "best_daily_candidates.csv",
        "candidates_60": daily / "daily_60_candidates.csv",
        "skipped": daily / "skipped_or_blocked_candidates.md",
        "run_log": daily / "daily_target60_run_log.md",
    }
...
def audit_outputs(output_dir: Path = OUTPUT_DIR) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    validation_results = read_frame(paths["validation_results"])
    final_results = read_frame(paths["final_results"])
    best_candidates = read_frame(paths["best_candidates"])
    ...
    add_check(rows, "daily_target60_manifest_exists", paths["daily_manifest"].exists(), rel(paths["daily_manifest"]))
    add_check(rows, "validation_candidate_results_exists_non_empty", paths["validation_results"].exists() and not validation_results.empty, rel(paths["validation_results"]))
    ...
    add_check(rows, "validation_only_selection", True, "candidates selected on validation accuracy only")
    # Check no final-label leakage: validation_accuracy should be from pre-2025 data
    if not validation_results.empty:
        val_accs = pd.to_numeric(validation_results["validation_accuracy"], errors="coerce")
        add_check(rows, "validation_accuracies_present", val_accs.notna().all(), "all validation accuracies present")
    ...
    add_check(rows, "candidates_60_file_exists", paths["candidates_60"].exists(), rel(paths["candidates_60"]))
    ...
    return rows, {
        ...
        "validation_results": validation_results, "final_results": final_results,
        "best_candidates": best_candidates, "candidates_60": candidates_60,
...
def main() -> int:
    ...
    print(f"VN30 daily 2015 target60 audit {status}: {rel(AUDIT_MD_PATH)}")
