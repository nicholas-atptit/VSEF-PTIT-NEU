"""Audit all-model final65 router results."""
...
BASELINE_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_all_model_final65_router"
REPORT_DIR = REPO_ROOT / "reports" / "generated" / "vn30_hourly_2015_all_model_final65_router"
AUDIT_CSV_PATH = REPORT_DIR / "all_model_final65_router_audit.csv"
AUDIT_MD_PATH = REPORT_DIR / "all_model_final65_router_audit.md"
...
def main() -> int:
    ...
    manifest = load_json(BASELINE_DIR / "all_model_router_manifest.json")
    eval_results = load_csv(BASELINE_DIR / "final_router_results.csv")
    ...
    add("baseline60_retained", True, "RF h=60 60.22% baseline retained", severity="info")
    ...
    global_65 = [r for r in eval_results if float(r.get("final_coverage", 0) or 0) >= 0.95 and float(r.get("final_accuracy", 0) or 0) >= 0.65]
    ...
    md = ["# VN30 Hourly 2015 - All-Model Final65 Router Audit", "",
        f"- Generated: {now_utc()}", f"- Audit: {pc} pass, {wc} warn, {fc} fail", "",
        "## Audit Results", "", markdown_table(["check", "status", "severity", "details"], audit_rows), "",
        "## Summary", "",
        f"- Final65 >=65: {'PASS' if f65_pass else 'FAIL'} (best: {fmt_pct(best_f65)})",
        f"- Gap to 65: {fmt_pct(0.65 - best_f65)}",
        f"- Global full-universe >=65: {'YES' if global_65 else 'NO'}",
        f"- Exploratory >=65: {'YES' if exploratory_65 else 'NO'}", ""]
    if best_qualified_row:
        md.extend(["## Best Final65 Candidate", "",
            f"- Policy: {best_qualified_row.get('policy_type', '')}",
            f"- Model: {best_qualified_row.get('model', '')}, Horizon: {best_qualified_row.get('horizon', '')}",
            f"- Target: {best_qualified_row.get('target_type', '')}",
            ...
            f"- Active Tickers: {best_qualified_row.get('active_ticker_count', '')}", ""])
    if selected_policies:
        md.extend(["## Selected Policy", "",
            markdown_table(["policy_id", "policy_type", "model", "horizon", "target_type", "validation_accuracy", "final_accuracy", "final_coverage", "final_rows"], selected_policies), ""])
