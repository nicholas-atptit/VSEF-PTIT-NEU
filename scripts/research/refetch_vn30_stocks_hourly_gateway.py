"""Refetch frozen VN30 stock hourly OHLCV through the gateway."""
...
RAW_ROOT = REPO_ROOT / "data" / "raw" / "vnstock_fetch" / "vn30_hourly_gateway"
CACHE_ROOT = REPO_ROOT / "data" / "market_cache" / "vnstock_data" / "vn30" / "hourly_gateway"
REPORT_ROOT = REPO_ROOT / "reports" / "generated" / "vn30_hourly_gateway" / "fetch"
SUMMARY_CSV = REPORT_ROOT / "vn30_hourly_gateway_fetch_summary.csv"
SUMMARY_MD = REPORT_ROOT / "vn30_hourly_gateway_fetch_summary.md"
FAILURES_CSV = REPORT_ROOT / "vn30_hourly_gateway_failures.csv"
CHUNK_LOG_CSV = REPORT_ROOT / "vn30_hourly_gateway_chunk_log.csv"
...
def read_listing_dates() -> dict[str, date]:
    if not LISTING_DATES_PATH.exists():
        return {}
    with LISTING_DATES_PATH.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    values: dict[str, date] = {}
    for row in rows:
        ticker = str(row.get("ticker", "")).strip().upper()
        value = str(row.get("first_trading_date", "")).strip()
        if not ticker or not value:
            continue
        parsed = pd.to_datetime(value, errors="coerce")
        if not pd.isna(parsed):
            values[ticker] = pd.Timestamp(parsed).date()
    return values
...
def summarize(ticker: str, frames: list[pd.DataFrame], stopped_reason: str, requested_start: date) -> dict[str, Any]:
...
def write_reports(summary: list[dict[str, Any]], failures: list[dict[str, Any]], logs: list[dict[str, Any]]) -> None:
    ...
    write_csv(SUMMARY_CSV, summary, ["ticker", "requested_start", "fetched", "rows", "first_timestamp", "last_timestamp", "provider", "source", "frequency", "cache_path", "stopped_reason"])
    ...
    lines = [
        "# VN30 Stock Hourly Gateway Fetch Summary",
        "",
        ...
        "- Resampling used: no.",
        "",
        "| ticker | fetched | rows | first | last | provider | source | frequency | stopped_reason |",
        "|---|---:|---:|---|---|---|---|---|---|",
    ]
    for row in summary:
        lines.append(f"| `{row['ticker']}` | {row['fetched']} | {row['rows']} | {row['first_timestamp']} | {row['last_timestamp']} | `{row['provider']}` | `{row['source']}` | `{row['frequency']}` | `{row['stopped_reason']}` |")
...
def main() -> int:
    ...
    listing_dates = read_listing_dates()
    ...
    for ticker in tickers:
        requested_start = max(listing_dates.get(ticker, fallback_start), fallback_start)
        stopped = "completed"
        frames: list[pd.DataFrame] = []
        ...
        summaries.append(summarize(ticker, frames, stopped, requested_start))
