"""Fetch supported Vietnamese index hourly OHLCV from 2015 through the gateway."""
...
def resume_command(code: str, args: argparse.Namespace) -> str:
    return (
        ...
        f"--index-code {code} --start {args.start} --end {args.end} --chunk-days {args.chunk_days} --resume "
        f"--max-runtime-seconds {args.max_runtime_seconds}"
    )


def summarize(code: str, frames: list[pd.DataFrame], stopped_reason: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "resume_command": resume_command(code, args) if stopped_reason.startswith("max_runtime_seconds=") else "",
    }


def write_reports(summary: list[dict[str, Any]], failures: list[dict[str, Any]], logs: list[dict[str, Any]]) -> None:
    REPORT_ROOT.mkdir(parents=True, exist_ok=True)
    write_csv(SUMMARY_CSV, summary, ["index_code", "fetched", "rows", "first_timestamp", "last_timestamp", "provider", "source", "frequency", "cache_path", "stopped_by_runtime_cap", "stopped_reason", "resume_command"])
    ...
    for row in summary:
        lines.append(f"| `{row['index_code']}` | {row['fetched']} | {row['rows']} | {row['first_timestamp']} | {row['last_timestamp']} | `{row['provider']}` | `{row['source']}` | `{row['frequency']}` | {row['stopped_by_runtime_cap']} | `{row['stopped_reason']}` |")
    ...
    for code in codes:
        ...
        for chunk_start, chunk_end in date_ranges(start, end, args.chunk_days):
            if args.max_runtime_seconds and time.monotonic() - started >= args.max_runtime_seconds:
                stopped = f"max_runtime_seconds={args.max_runtime_seconds}" if started_code else "not_started_runtime_cap"
                break
            started_code = True
        ...
        summaries.append(summarize(code, frames, stopped, args))
