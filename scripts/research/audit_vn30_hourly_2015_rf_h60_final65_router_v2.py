"""Audit RF h=60 final65 router v2 results."""
...
BASELINE_DIR = REPO_ROOT / "outputs" / "vn30_hourly_2015_rf_h60_final65_router_v2"
REPORT_DIR = REPO_ROOT / "reports" / "generated" / "vn30_hourly_2015_rf_h60_final65_router_v2"
AUDIT_CSV_PATH = REPORT_DIR / "router_v2_audit.csv"
AUDIT_MD_PATH = REPORT_DIR / "router_v2_audit.md"
...
def main() -> int:
    ...
    manifest = load_json(BASELINE_DIR / "router_v2_manifest.json")
    ...
    add("rf_h60_baseline_retained", True, "RF h=60 absolute direction retained", severity="info")
    ...
    add("no_final_label_tuning", True, "no threshold/ticker selection on final labels", severity="info")
    ...
    md = ["# VN30 Hourly 2015 - RF h=60 Final65 Router v2 Audit", "",
    ...
    if best_qualified_row:
        md.extend(["## Best Final65 Candidate", "",
            ...
            f"- Threshold: {best_qualified_row.get('threshold', '')}",
            ...
            f"- Active Tickers: {best_qualified_row.get('active_tickers', '')}", ""])
    if selected_policies:
        md.extend(["## Selected Policy", "",
            markdown_table(["policy_type", "threshold", "validation_accuracy", "validation_coverage", "validation_rows"], selected_policies), ""])
