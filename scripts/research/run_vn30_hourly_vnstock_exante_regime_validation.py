"""Validate vnstock-fetched VN30 hourly predictions with ex-ante regime labels."""
...
DEFAULT_OUTPUT_DIR = FULL_REPORT_ROOT / "regime"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VN30 hourly vnstock ex-ante regime validation.")
...
def write_report(path: Path, artifact_dir: Path, summary: pd.DataFrame) -> None:
    ...
    content = [
        "# VN30 Hourly vnstock Ex-Ante Regime Validation Report",
        "",
        "## Source",
        "",
    ...
    if summary.empty:
        content.append("No ex-ante regime claims are available because vnstock full-benchmark predictions are missing or empty.")
    else:
...
def write_plot(path: Path, summary: pd.DataFrame) -> str:
    if summary.empty:
        return save_placeholder_figure(path, "VN30 hourly vnstock ex-ante regime accuracy", "No hourly predictions were available.")
    try:
        import matplotlib
        ...
        if aggregate.empty:
            return save_placeholder_figure(path, "VN30 hourly vnstock ex-ante regime accuracy", "No ex-ante aggregate rows were available.")
        ...
        ax.set_title("VN30 hourly vnstock ex-ante regime accuracy")
        ...
        return save_placeholder_figure(path, "VN30 hourly vnstock ex-ante regime accuracy", f"Plot rendering failed: {exc}")
...
def main() -> int:
    ...
    summary_path = args.output_dir / "vn30_hourly_vnstock_exante_regime_accuracy_summary.csv"
    report_path = args.output_dir / "vn30_hourly_vnstock_exante_regime_validation_report.md"
    figure_path = args.output_dir / "vn30_hourly_vnstock_regime_accuracy.png"
    summary.to_csv(summary_path, index=False)
    write_report(report_path, args.artifact_dir, summary)
    figure_status = write_plot(figure_path, summary)
    print(
        "VN30 hourly vnstock ex-ante regime validation complete: "
