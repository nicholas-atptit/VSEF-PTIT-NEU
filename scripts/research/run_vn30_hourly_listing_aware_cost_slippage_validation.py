"""Run proxy cost/slippage diagnostics for the VN30 listing-aware hourly benchmark."""
...
DEFAULT_CONFIDENCE_PATH = REPORT_ROOT / "confidence" / "vn30_listing_aware_confidence_sweep_summary.csv"
DEFAULT_REGIME_PATH = REPORT_ROOT / "regime" / "vn30_listing_aware_exante_regime_accuracy_summary.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VN30 listing-aware cost/slippage diagnostics.")
...
def write_report(path: Path, artifact_dir: Path, summary: pd.DataFrame) -> None:
    ...
    content = [
        "# VN30 Hourly Listing-Aware Cost/Slippage Proxy Validation Report",
        "",
        "## Source",
        "",
...
def write_plot(path: Path, equity_curve: pd.DataFrame) -> str:
    if equity_curve.empty:
        return save_placeholder_figure(path, "VN30 listing-aware proxy equity curve", "No selected candidates were available for diagnostics.")
    try:
        import matplotlib
        ...
        ax.set_title("VN30 listing-aware cost/slippage proxy equity curve")
        ...
        return save_placeholder_figure(path, "VN30 listing-aware proxy equity curve", f"Plot rendering failed: {exc}")


def main() -> int:
    args = parse_args()
    summary, trade_list, equity_curve = run_diagnostics(
        load_hourly_predictions(args.artifact_dir),
    ...
    summary_path = args.output_dir / "vn30_listing_aware_cost_slippage_summary.csv"
    report_path = args.output_dir / "vn30_listing_aware_cost_slippage_validation_report.md"
    trade_path = args.output_dir / "vn30_listing_aware_trade_list.csv"
    equity_path = args.output_dir / "vn30_listing_aware_equity_curve.csv"
    figure_path = args.output_dir / "vn30_listing_aware_equity_curve.png"
    summary.to_csv(summary_path, index=False)
    trade_list.to_csv(trade_path, index=False)
    equity_curve.to_csv(equity_path, index=False)
    write_report(report_path, args.artifact_dir, summary)
    status = write_plot(figure_path, equity_curve)
    print(f"VN30 listing-aware cost/slippage diagnostics complete: rows={len(summary)} report={rel(report_path)} figure_status={status}")
    return 0
