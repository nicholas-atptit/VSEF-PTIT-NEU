"""Run the multi-horizon forward-return backtest on real vnstock data."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Multi-horizon forward-return backtest on real vnstock data")
    parser.add_argument(
        "--tickers",
        nargs="+",
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="artifacts/backtest_forward_return",
    ...
    parser.add_argument(
        ...
        type=str,
        default="3d,5d,20d",
        help="Comma-separated forward-return horizons to evaluate",
    )
    parser.add_argument(
        "--algorithms",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable the optional momentum continuation baseline",
    )
...
def main() -> None:
    ...
    print("Forward-return backtest completed.")
    ...
    for horizon, horizon_result in result["horizons"].items():
        ...
        for name, path in horizon_result["paths"].items():
            print(f"{name}: {path}")
        preview = horizon_result["metrics"][
            ["ticker", "model_name", "mae", "rmse", "mape", "directional_accuracy", "beats_naive_baseline"]
        ].head(12)
        print(preview.round(6).to_string(index=False))

    print("\nCross-horizon summary file:")
    for name, path in result["overall_paths"].items():
        print(f"{name}: {path}")

    print("\nOverall horizon ranking:")
