...
class XgboostModel(BaseModel):
    """XGBoost wrapper matching the unified ML inference facade."""
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if not HAS_XGBOOST:
            raise ImportError("xgboost is not installed. Run process with optional dependencies installed.")
    ...
    def _build_estimator(self) -> Any:
        ...
        if self.task == "classification":
            return xgb.XGBClassifier(**valid_params, eval_metric="logloss")
        if self.task == "regression":
            return xgb.XGBRegressor(**valid_params, eval_metric="rmse")
    ...
    def predict_proba(self, X: Any) -> np.ndarray:
        if self.task != "classification":
            raise NotImplementedError("XGBoost regression models do not expose predict_proba")
    ...
    @classmethod
    def load(cls, artifact_path: Path) -> "XgboostModel":
