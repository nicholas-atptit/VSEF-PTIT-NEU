...
The empirical scope is limited to the official artifact family `outputs/vn100_hybrid_official_2025_confidence_sweep_traincutoff`. The model set consists of LightGBM, XGBoost, random forest, and stacking. No new model families are introduced in this paper draft.
...
The official benchmark evaluates LightGBM, XGBoost, random forest, and stacking. Random forests are tree ensembles based on bootstrap aggregation and feature randomness. XGBoost and LightGBM are gradient-boosting methods designed to fit additive trees sequentially. These models are common choices for tabular financial prediction because they can represent nonlinear interactions and handle mixed technical indicators without requiring sequence architectures.
...
Trading practicality requires more evidence than directional accuracy. A practical trading claim would require executable signal rules, entry and exit conventions, transaction costs, slippage, liquidity assumptions, turnover, drawdown, profit factor, equity curves, and cost-adjusted returns.
...
The final manuscript should add complete bibliographic entries for the following reference groups:

- Walk-forward and time-series validation literature.
- Random forest, XGBoost, LightGBM, and stacking ensemble literature.
- Directional accuracy and forecast evaluation literature.
- Statistical testing literature for forecast comparison and binomial hit-rate evaluation.
- Transaction cost, slippage, and trading practicality literature.

Candidate citation anchors from the outline include Breiman (2001), Wolpert (1992), Chen and Guestrin (2016), Ke et al. (2017), Christoffersen and Diebold (2006), Diebold and Mariano (1995), Bergmeir and Benitez (2012), Hyndman and Athanasopoulos (2021), and Almgren and Chriss (2001).
